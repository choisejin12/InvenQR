...
<b>현재도 많은 기업에서 오프라인 기반의 재고 관리 방식을 유지하고 있으며, 이는 데이터 불일치, 커뮤니케이션 오류, 작업 지연 등의 문제로 이어지고 있습니다.</b>
</br>이에 따라 본 프로젝트는 다음과 같은 문제를 해결하는 데 초점을 맞추었습니다.
...
* Prisma 스키마를 기준으로 핵심 관계만 요약한 구조
특히 `Warehouse -> Location -> Product -> InventoryLog` 흐름을 중심으로 재고와 이력을 연결하였음.
...
### ❌ Redux state 업데이트 안되는 문제
원인: reducer에서 state 변경 로직 누락</br>
해결: 
```ts
...
원인: 현재 Base64 업로드 구조라 이미지 크기가 너무 커서 발생</br>
해결: PayloadTooLargeError를 근본 해결한 게 아니라, 일단 서버 허용 크기를 늘려서 막았음
```ts
app.use(express.json({ limit: '10mb' }));
app.use(express.urlencoded({ extended: true, limit: '10mb' }));
```
추후 S3 같은 외부 스토리지 방식으로 분리하는 것이 더 적합할 것 같음.
