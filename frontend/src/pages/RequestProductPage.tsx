...
const RequestProductPage = () => {
  ...
  const handleRequestClick = (request: ProductRequestItem) => {
    ...
    navigate(`/product/${request.productId}`);
  };
      ...
      <section className="space-y-4">
        {isLoading ? (
          ...
          filteredRequests.map((request) => {
            const canNavigate = request.status === 'APPROVED' && !!request.productId;

            return (
              <article
                key={request.id}
                onClick={() => handleRequestClick(request)}
                className={`overflow-hidden border border-slate-200 bg-white shadow-sm transition ${
